...
// program name and description -----------------------------------------------
    ...
    CSO_PROG_DESC_BEGIN
    "Generate multiple-xyz file containing the structure and probe for vdW parametrization.\n"
    "\n"
    "<b>Supported generators:</b>\n"
    "  * random       - select random points (<b>--trials</b>) from grid constructed around molecule (<b>--buffer</b>) with <b>--spacing</b>\n"
    "  * grid         - select all points from grid constructed around molecule (<b>--buffer</b>) with <b>--spacing</b>\n"
    "  * grid-closest - similar as <u>grid-closest</u> but only closest grid points to selected atoms are considered\n"
    "  * msms-all     - vertices generated by the msms program and provided via <b>--msms</b>,\n"
    "                   only vertices belonging to selected atoms are considered\n"
    "  * msms-random-per-atom\n"
    "                   - similar as <u>msms-all</u> but vertices are selected randomly from surface facets of selected atoms\n"
    "                   number of trials is given by <b>--trials</b> on facets with minimum <b>--maxprobes</b> vertices\n"
    "  * msms-random-per-atom-with-min-prefilter\n"
    "                 - similar as <u>msms-random-per-atom</u> but vertices are prefiltered with the minonly filter,\n"
    "                   which is suitable for situations when the surface is generated for other structure then <u>STRUCTURE</u>\n"
    "  * msms-random-per-all\n"
    "                   - similar as <u>msms-all</u> but vertices are selected randomly from the entire surface\n"
    "                   number of trials is given by <b>--trials</b> on facets with minimum <b>--maxprobes</b> vertices\n"
    "  * msms-random-per-all-with-min-prefilter\n"
    "                 - similar as <u>msms-random-per-all</u> but vertices are pre-filtered with the minonly filter,\n"
    "                   which is suitable for situations when the surface is generated for other structure then <u>STRUCTURE</u>\n"
    ...
    "  * sphere-max-distance - select those vertices on discretized spheres constructed around selected atoms\n"
    "                   that exhibit maximum distance to all other atoms\n"
    "                   spheres are constructed with radius ranging from <b>--min</b> to <b>--max</b> with <b>--spacing</b>\n"
