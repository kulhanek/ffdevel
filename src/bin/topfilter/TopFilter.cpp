...
{
    vout << endl;
    vout << ">>> FILTER: ldihedrals - remove straight dihedrals from topology" << endl;
    ...
    for(int i=0; i < Topology.DihedralList.GetNumberOfDihedrals(); i++){
        ...
        if( (value1 > (180.0-10.0)*M_PI/180.0) || (value2 > (180.0-10.0)*M_PI/180.0) ){
            dih.tidx = -1;
            vout << "       ... found straight dihedral id " << i+1 << " value ";

            if( value1 > (180.0-10.0)*M_PI/180.0 ) {
    ...
    vout << "    Number of straight dihedrals : " << nsa << endl;
    ...
    for(int i=0; i < Topology.DihedralList.GetNumberOfDihedralTypes(); i++){
        ...
        if( found == false ){
            ...
            vout << "       ... found dihedral type id " << i+1 << " participated only in straight dihedrals" << endl;
            nsat++;
        }
        dtypes.push_back(dtype);
    }
    vout << "    Number of straight dihedral types : " << nsat << endl;
