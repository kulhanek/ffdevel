...
subroutine ffdev_hessian_calc_trg_freqs(geo)
    ...
    allocate(geo%trg_freq(3*geo%natoms), geo%trg_nmodes(3,geo%natoms,3,geo%natoms), &
             geo%freq_t2s_map(3*geo%natoms), geo%freq_t2s_angles(3*geo%natoms), &
             geo%freq_t2s_rmsd(3*geo%natoms), stat = alloc_stat)
    if( alloc_stat .ne. 0 ) then
    ...
    geo%freq_t2s_map(:) = 0
    geo%freq_t2s_angles(:) = 0.0d0
    geo%freq_t2s_rmsd(:) = 0.0d0
...
!===============================================================================
! subroutine:  ffdev_hessian_superimpose_freqs_by_geo
!===============================================================================

subroutine ffdev_hessian_superimpose_freqs_by_geo(natoms,z,ref,src,srcnmodes,mw,rmsd)
...
end subroutine ffdev_hessian_superimpose_freqs_by_geo

!===============================================================================
! subroutine:  ffdev_hessian_superimpose_mode
!===============================================================================

subroutine ffdev_hessian_superimpose_freqs_by_mode(refidx,natoms,ref,src,refnmodes,srcnmodes,rmsd)
    ...
    real(DEVDP)     :: ref(:,:)
    real(DEVDP)     :: src(:,:)
    real(DEVDP)     :: refnmodes(:,:,:,:)
    real(DEVDP)     :: srcnmodes(:,:,:,:)
    real(DEVDP)     :: rmsd
    ! --------------------------------------------
    integer         :: i,info,best,j,k,mi,mj
    real(DEVDP)     :: x1,x2,x3,xr1,xr2,xr3,amass,totmass,itotmass
    real(DEVDP)     :: r11,r12,r13,r21,r22,r23,r31,r32,r33
    real(DEVDP)     :: f(4,4),u(3,3)
    real(DEVDP)     :: eigenvalues(4),work(26*4)
    ...
    do  i = 1, natoms
        amass = refnmodes(1,i,mj,mi)**2 + refnmodes(2,i,mj,mi)**2 + refnmodes(3,i,mj,mi)**2
        ! source
        x1 = x1 + src(1,i)*amass
        x2 = x2 + src(2,i)*amass
        x3 = x3 + src(3,i)*amass
...
end subroutine ffdev_hessian_superimpose_freqs_by_mode

!===============================================================================
! subroutine:  ffdev_hessian_superimpose_by_geo
!===============================================================================

subroutine ffdev_hessian_superimpose_by_geo(geo)
    ...
    call ffdev_hessian_superimpose_freqs_by_geo(geo%natoms,geo%z,geo%trg_crd, &
                                         geo%crd,geo%nmodes,.false.,rmsd)
    geo%freq_t2s_rmsd(:) = rmsd

    call ffdev_hessian_find_mapping_by_angle(geo%natoms,geo%trg_nmodes, &
                                    geo%nmodes,geo%freq_t2s_map, &
                                    geo%freq_t2s_angles)

end subroutine ffdev_hessian_superimpose_by_geo

!===============================================================================
! subroutine:  ffdev_hessian_superimpose_by_modes
!===============================================================================

subroutine ffdev_hessian_superimpose_by_modes(geo)
    ...
    do t=7,3*geo%natoms
        ! superimpose
        call ffdev_hessian_superimpose_freqs_by_mode(t,geo%natoms,geo%trg_crd,geo%crd, &
                                             geo%trg_nmodes,geo%nmodes,rmsd)
        geo%freq_t2s_rmsd(t) = rmsd

        ! and then find mapping
        call ffdev_hessian_find_mapping_by_mode(t,geo%natoms,geo%trg_nmodes, &
                                        geo%nmodes,geo%freq_t2s_map, &
                                        geo%freq_t2s_angles)
    end do

end subroutine ffdev_hessian_superimpose_by_modes

!===============================================================================
! subroutine:  ffdev_hessian_find_mapping_by_angle
!===============================================================================

subroutine ffdev_hessian_find_mapping_by_angle(natoms,nmodes1,nmodes2,map1t2,map1t2alpha)
    ...
    real(DEVDP)         :: nmodes2(:,:,:,:)
    integer             :: map1t2(:)
    real(DEVDP)         :: map1t2alpha(:)
    ! -------------------------------------------
    integer             :: alloc_stat, t, f, i1, j1, i2, j2, min_j, k, l
    real(DEVDP)         :: min_angle, angle, n1, n2
    ...
    ! search in reverse order,i.e., from more localized vibs
    do t = 3*natoms,7,-1
        i1 = (t - 1) / 3 + 1
        j1 = mod(t - 1,3) + 1

        min_j = 0
        min_angle = 0.5d0*DEV_PI

        ! search in reverse order,i.e., from more localized vibs
        ! the idea is that high frequency vibrations are more localized and thus easily match
        do f = 3*natoms,7,-1
            if( map1(f) .gt. 0 ) cycle  ! already assigned

            i2 = (f - 1) / 3 + 1
            j2 = mod(f - 1,3) + 1

            ! calculate angle
            ! modes should be already normalized but anyway, we will calculate norms
            angle = 0.0d0
            n1 = 0.0d0
            n2 = 0.0d0
            do k=1,natoms
                do l=1,3
                    ...
                    n1 = n1 + nmodes1(l,k,j1,i1)**2
                    n2 = n2 + nmodes2(l,k,j2,i2)**2
                end do
            end do
            if( n1*n2 .gt. 0.0d0 ) then
                angle = angle / sqrt(n1*n2)
            end if
            angle = acos(angle)
            ! write(*,*) angle*DEV_R2D
            ! we are searching for angle close to zero or 180
            if( angle .gt. 0.5d0*DEV_PI ) then
                angle = DEV_PI - angle
            end if
            if( (angle .lt. min_angle) .or. (min_j .eq. 0) ) then
                min_angle = angle
                min_j = f
            end if
        end do
        map1(min_j) = t
        map1t2(t) = min_j
        map1t2alpha(t) = min_angle
    end do

    deallocate(map1)

end subroutine ffdev_hessian_find_mapping_by_angle

!===============================================================================
! subroutine:  ffdev_hessian_find_mapping_by_mode
!===============================================================================

subroutine ffdev_hessian_find_mapping_by_mode(tmode,natoms,nmodes1,nmodes2,map1t2,map1t2alpha)
    ...
    real(DEVDP)         :: map1t2alpha(:)
    ! -------------------------------------------
    integer             :: alloc_stat, t, f, i1, j1, i2, j2, min_j, k, l, g
    real(DEVDP)         :: min_angle, angle, n1, n2
    logical             :: found
    ! --------------------------------------------------------------------------

    ! search for given mode
    t = tmode
        i1 = (t - 1) / 3 + 1
        j1 = mod(t - 1,3) + 1
        ...
        do f = 3*natoms,7,-1
            found = .false.
            do g=3*natoms,7
                if( map1t2(g) .eq. f ) then
                    found = .true.
                    exit
                end if
            end do
            if( found ) cycle ! already assigned
...
end subroutine ffdev_hessian_find_mapping_by_mode

!===============================================================================
! subroutine:  ffdev_hessian_print_mapping
!===============================================================================

subroutine ffdev_hessian_print_mapping(c12,natoms,freq1,nmodes1,freq2,nmodes2,map1t2,map1t2alpha,map1t2rmsd)
    ...
    integer             :: map1t2(:)
    real(DEVDP)         :: map1t2alpha(:)
    real(DEVDP)         :: map1t2rmsd(:)
    ! -------------------------------------------
    integer             :: i, i1, j1, i2, j2, k, l
    real(DEVDP)         :: serr, lerr, aerr, rmse, angle, diff, avangle
    ! --------------------------------------------------------------------------
    ...
    do i=7,3*natoms
        ...
        angle = map1t2alpha(i)*DEV_R2D

        write(DEV_OUT,140) i, freq1(i), map1t2(i), freq2(map1t2(i)), angle, map1t2rmsd(i), diff
    ...
    if( 3*natoms - 6 .gt. 0 ) then
        ...
        avangle = avangle / real(3*natoms - 6)
    end if
    ...
    write(DEV_OUT,190) avangle

100 format('# Individual vibrations')
120 format('# Indx#1  Freq#1  Indx#2  Freq#2    Angle    RMSD Diff')
125 format('# Id#TRG Freq#TRG  Id#MM  Freq#MM   Angle    RMSD Diff')
130 format('# ------ -------- ------ -------- ------- ------- ------')
140 format(I8,1X,F8.2,1X,I6,1X,F8.2,1X,F7.1,1X,F7.2,1X,F6.1)
...
190 format('# Average angle (AA)                 = ',F9.4)
