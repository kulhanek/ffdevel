...
subroutine ffdev_hessian_allocate_freq(geo)
    ...
    allocate(geo%freq(3*geo%natoms), stat = alloc_stat)
    if( alloc_stat .ne. 0 ) then
        call ffdev_utils_exit(DEV_OUT,1,'Unable to allocate array for frequencies!')
    end if
        ...
        do k=1,geo%natoms
            do l=1,3
                ...
                do h=1,r
                    i = (m+h-1) / 3 + 1
                    j = mod(m+h-1,3) + 1
                    write(fout,50,ADVANCE='NO') geo%hess(l,k,j,i)
                end do
                write(fout,*)
            end do
        end do
...
subroutine ffdev_hessian_calc_freqs(geo)
    ...
    do i=1,geo%natoms
        ...
        do j=1,3
            do k=1,geo%natoms
                ...
                do l=1,3
                    geo%hess(j,i,l,k) =  geo%hess(j,i,l,k)*m1*m2
                end do
            end do
        end do
    end do
    ...
    lwork = -1
    call dsyev('V','L',geo%natoms*3, geo%hess, geo%natoms*3, geo%freq, tmp, lwork, info)
    lwork = tmp(1)
    ...
    ! diagonalize matrix
    call dsyev('V','L',geo%natoms*3, geo%hess, geo%natoms*3, geo%freq, work, lwork, info)
