...
!                    sw = 1.0d0 / (1.0d0 + exp( RMSDSWAlpha*(dt - RMSDSWPosition) ) )
!                    swsum = swsum + sw
!                    err = err * sw
!                    seterrrmsd = seterrrmsd + sets(i)%geo(j)%weight * err**2
!                end do
!            end if
!        end do
!    end do
...
subroutine ffdev_err_rmsd_summary(top,geo)
...
!        ! calculate switch function
!        sw = 1.0d0 / (1.0d0 + exp( RMSDSWAlpha*(d1 - RMSDSWPosition) ) )
!        swsum = swsum + sw
!        sdiff = diff * sw
